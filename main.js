...
while (birthMonth + '/'+ birthDate + '/' + birthYear); { 
        if (birthYear < getUTCFullYearz() - 21); {
             console.log(true);
                alert('Phew, you had me worried there');
        }
        if (birthYear > (getUTCFullYear() - 21)); {
            console.log(false);
            alert('Wait...Hold up. If your birthday is ' + birthMonth + '/'+ birthDate + '/' + birthYear + ' then how are you 21?');   
        }
        if ((((birthYear == (getUTCFullYear() - 21)) && birthMonth <= getUTCMonth() && birthDate <= getUTCDate())));{
             console.log(true); 
                alert('Phew, you had me worried there');
        } 
         if ((((birthYear == (getUTCFullYear() - 21)) && birthMonth >= getUTCMonth() && birthDate > (getUTCDate() + 1) || birthYear > (getUTCFullYear()-21)))); {
             console.log(false);
            alert('Wait...Hold up. If your birthday is ' + birthMonth + '/'+ birthDate + '/' + birthYear + ' then how are you 21?');    
        }   
};
